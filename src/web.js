      ...
      if (
        ...
        if (val !== val) {
          ...
          log(
            'WARNING: Freshpaint ' +
              destType +
              " GTM Template could not parse prop '" +
              prop.name +
              "' as numeric, leaving as string: " +
              val,
          );
        }
...
const generateOptionsFromInstances = (integration, instanceNames, supportMulti) => {
  ...
  if (instanceNamesToUse) {
    ...
    for (let i = 0; i < instanceNamesToUse.length; i++) {
      ...
      integrations[integration + instanceDelimiter + instanceNamesToUse[i].toString().trim()] =
        true;
    }
...
const processTrack = () => {
  if (data.commonEventName) {
    const props = parseSimpleTableAndParseNumericAndJSONValues(
      data.commonEventPropertiesJSONValue || [],
      'track',
    );

    const options = generateOptionsFromParamTable(
      data.commonOptinOptOut,
      data.commonOptinOptOutInstances,
    );
...
const processIdentify = () => {
  const options = generateOptionsFromParamTable(
    data.commonOptinOptOut,
    data.identifyOptinOptOutInstances,
  );
  ...
  const eventName =
    data.fbEventName === 'custom'
      ...
      : data.fbEventName === 'variable'
        ? data.fbVariableEventName
        : data.fbStandardEventName;
  const objectProps =
    ...
    if (propValue) {
      if (propKey === 'value' || propKey === 'num_items') {
        ...
        if (val !== val) {
          ...
          val = propValue;
          log(
            "WARNING: Freshpaint Facebook Conversions API GTM Template could not parse prop '" +
              propKey +
              "' as numeric, leaving as string: " +
              propValue,
          );
        ...
        if (!val) {
          log(
            "WARNING: Freshpaint Facebook Conversions API GTM Template could not parse '" +
              propKey +
              "' json, leaving as string: " +
              propValue,
          );
          val = propValue;
        }
    ...
    if (propValue) {
      if (propKey === 'value') {
        ...
        if (val !== val) {
          ...
          val = propValue;
          log(
            "WARNING: Freshpaint TikTok Ads GTM Template could not parse prop '" +
              propKey +
              "' as numeric, leaving as string: " +
              propValue,
          );
        ...
        if (!val) {
          log(
            "WARNING: Freshpaint TikTok Ads GTM Template could not parse '" +
              propKey +
              "' json, leaving as string: " +
              propValue,
          );
...
const processBingEvent = () => {
  ...
  if (data.bingEventType === 'VARIABLE_REVENUE') {
    ...
    data.bingEventType === 'ecommerce' ||
    data.bingEventType === 'hotel' ||
    data.bingEventType === 'travel'
  ) {
...
const processImpactEvent = () => {
  ...
  for (let key in props) {
    ...
    if (props[key] !== props[key]) {
      // Check for NaN
      log(
        "WARNING: Freshpaint impact.com GTM Template could not parse prop '" +
          key +
          "' as numeric, leaving as string: " +
          props[key],
      );
  ...
  if (itemsVal) {
    ...
    if (!itemsVal) {
      log(
        'WARNING: Freshpaint impact.com GTM Template parsing items json, leaving as string: ' +
          props.items,
      );
...
const constructECUserDataProps = (ecUserData) => {
  ...
  if (ecUserData && typeof ecUserData === 'object') {
    ...
    if (
      ecUserData.address &&
      typeof ecUserData.address === 'object' &&
      ecUserData.address[0] &&
      typeof ecUserData.address[0] === 'object'
    ) {
...
const processGoogleAdsCallConversionsEvent = () => {
  let tagIdConversionLabel =
    'AW-' + data.googleAdsCallConversionsConversionId + '/' + data.googleAdsConversionLabel;
...
const processLinkedInAdsEvent = () => {
  ...
  if (conversionIds.length === 0) {
    log(
      'ERROR: Freshpaint LinkedIn Ads GTM Template missing Conversion ID(s): ' +
        data.commonEventName,
    );
    data.gtmOnFailure();
    return;
  } else if (conversionIds.length > 3) {
    log(
      'ERROR: Freshpaint LinkedIn Ads GTM Template supports only up to 3 Conversion IDs: ' +
        data.commonEventName,
    );
    data.gtmOnFailure();
    return;
...
const processLinkedInAdsCAPIEvent = () => {
  ...
  if (conversionIds.length === 0) {
    log(
      'ERROR: Freshpaint LinkedIn Ads Conversions API GTM Template missing Conversion ID(s): ' +
        data.commonEventName,
    );
    data.gtmOnFailure();
    return;
    ...
    log(
      'ERROR: Freshpaint LinkedIn Ads Conversions API GTM Template supports only up to 3 Conversion IDs: ' +
        data.commonEventName,
    );
    data.gtmOnFailure();
    return;
  ...
  if (data.commonEventName && data.theTradeDeskTrackerOrUPixelIDValue) {
    ...
    if (data.theTradeDeskValue) {
      ...
      if (val !== val) {
        ...
        log(
          'WARNING: Freshpaint theTradeDesk GTM Template could not parse prop value as numeric, leaving as string: ' +
            val,
        );
      }
      props.value = val;
      ...
      if (!val) {
        ...
        log(
          'WARNING: Freshpaint ' +
            theTradeDeskSDKKey +
            " GTM Template could not parse prop '" +
            data.theTradeDeskPrivacySettings +
            "' as JSON, leaving as string: " +
            val,
        );
      }
      ...
      if (!val) {
        ...
        log(
          'WARNING: Freshpaint ' +
            theTradeDeskSDKKey +
            " GTM Template could not parse prop '" +
            data.theTradeDeskDataProcessingOption +
            "' as JSON, leaving as string: " +
            val,
        );
      }
    ...
    log(
      'ERROR: Freshpaint theTradeDesk GTM Template missing eventName and / or trackerOrUPixelIDValue',
    );
    data.gtmOnFailure();
  }
};
...
const processStackAdaptEvent = () => {
  ...
  if (data.commonEventName && data.stackAdaptConversionEventID) {
    ...
    log(
      'ERROR: Freshpaint StackAdapt GTM Template missing eventName and / or stackAdaptConversionEventID',
    );
    data.gtmOnFailure();
  }
};
...
const processFloodlightEvent = () => {
  ...
  let props = parseParamTable(data.floodlightCustomVariables || [], {
    keyColumnName: 'key',
